...
matplotlib.rcParams["image.cmap"] = 'turbo'
import matplotlib.ticker as tick



with np.load('accelerated_Ny32Re600.npz') as data:
    U = data['U']
    V = data['V']
...
levels=np.linspace(np.min(psi),np.max(psi),35)
tu,tl= wall_shear(U,h,x)


figname=[r'contour_Ny%dRe%d'%(Ny,Re),r'shear_Ny%dRe%d'%(Ny,Re)]
plt.figure(figsize=(10, 4), dpi=100)
plt.title('$Flow\ Field\ (Re=%d,\ Ny=%d)$'%(Re,Ny))
plt.contour(X,Y,psi,levels=levels, linewidths=1, colors='w')
